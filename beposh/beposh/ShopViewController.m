...
//  Copyright © 2016年 TYZ. All rights reserved.  @"http://www.beposh.net"
//
...
- (void)viewDidLoad {
    ...
    self.ProgressView.progressTintColor = [UIColor colorWithRed:(0x00)/255.0f green:(0xbe)/255.0f blue:(0xa4)/255.0f alpha:1];
...
- (void)startLoad
{
    NSString *url_str = [NSString stringWithFormat:@"http://www.beposh.net"];
...
- (void)webView:(WKWebView *)webView didFailProvisionalNavigation:(WKNavigation *)navigation withError:(NSError *)error {
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void)
                   {
                       //加载失败同样需要隐藏progressView   - > NetWork Error!
                       self.ProgressView.hidden = YES;
