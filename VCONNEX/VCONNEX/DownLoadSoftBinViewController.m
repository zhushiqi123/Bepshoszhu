...
-(BOOL)navigationShouldPopOnBackButton ///在这个方法里写返回按钮的事件处理
{
    ...
    for (UIViewController *Settingcontroller in self.navigationController.viewControllers)
    {
        if ([Settingcontroller isKindOfClass:[SettingViewController class]])
        {
...
-(void)viewWillDisappear:(BOOL)animated
{
    ...
    {
        [ProgressHUD showError:@"Cancel Update"];
    }
}
...
-(void)addProgressView
{
    ...
    progressView01.backgroundColor = [UIColor groupTableViewBackgroundColor];//RGBCOLOR(0x74, 0x74, 0x74); //[UIColor darkGrayColor];
    progressView02.backgroundColor = [UIColor groupTableViewBackgroundColor];//RGBCOLOR(0x74, 0x74, 0x74); //[UIColor darkGrayColor];
    progressView03.backgroundColor = [UIColor groupTableViewBackgroundColor];//RGBCOLOR(0x74, 0x74, 0x74); //[UIColor darkGrayColor];
    progressView04.backgroundColor = [UIColor groupTableViewBackgroundColor];//RGBCOLOR(0x74, 0x74, 0x74); //[UIColor darkGrayColor];
    progressView05.backgroundColor = [UIColor groupTableViewBackgroundColor];//RGBCOLOR(0x74, 0x74, 0x74); //[UIColor darkGrayColor];
    progressView06.backgroundColor = [UIColor groupTableViewBackgroundColor];//RGBCOLOR(0x74, 0x74, 0x74); //[UIColor darkGrayColor];
...
-(void)addLableToPageNumView
{
    ...
    pageNumLable.textColor = [UIColor blackColor];
    
    pageNumLable.text = @"Hint:";
...
-(void)addLableToLogView
{
    ...
    LogLable.textColor = [UIColor blackColor];
    
    LogLable.text = @"Check the update file";
...
-(void)addTextView
{
    UIView *bg_view = [[UIView alloc]initWithFrame:CGRectMake(10, 200, SCREEN_WIDTH - 20, SCREEN_HEIGHT - 200 - 10.0f)];
    
    bg_view.backgroundColor = [UIColor groupTableViewBackgroundColor];
    
    //设置圆角
    bg_view.layer.cornerRadius = 10.0f;
    
    bg_view.layer.borderWidth = 2;
    
    bg_view.layer.borderColor = RGBACOLOR(0x74, 0x74, 0x74, 0.7).CGColor;
    
    [self.view addSubview:bg_view];
    
    UITextView *textViews = [[UITextView alloc]initWithFrame:CGRectMake(10, 10, bg_view.frame.size.width - 20.0f,bg_view.frame.size.height - 20.0f) textContainer:nil];

    //设置背景
    textViews.backgroundColor = [UIColor clearColor];
    ...
    //设置文字属性
    textViews.font = [UIFont systemFontOfSize:16.0f];
    ...
    //    textViews.layer.borderColor = RGBACOLOR(0x74, 0x74, 0x74, 0.7).CGColor;
    
    NSString *str_lable_text = [NSString stringWithFormat:@"1.Soft Name：%@\n\n2.Device Version：%@\n\n3.File Name：%@\n\n*Special Note：\n\n1.Do not disconnect the Bluetooth during the upgrade process.\n\n2.Do not leave this page during the upgrade process.\n\n3.Do not operate during the upgrade electronic cigarette, and keep the distance between the electronic cigarette and the phone.\n\n4.If the upgrade fails, go back to the installation package selection page and reinstall it.",[STW_BLE_SDK STW_SDK].softUpdate_bean.softName,[STW_BLE_SDK STW_SDK].softUpdate_bean.devieName,[STW_BLE_SDK STW_SDK].softUpdate_bean.binname];
    
    //        NSString *str_lable_text = [NSString stringWithFormat:@"1.Soft Name：%@\n\n2.Device Version：%@\n\n3.File Name：%@\n\n*特别提示：\n\n1.升级过程中请勿断开蓝牙；\n\n2.升级过程中请勿离开此页面；\n\n3.升级过程中请勿操作电子烟，并保持电子烟与手机的距离；\n\n4.升级失败请回退到安装包选择页面，重新进行安装；",[STW_BLE_SDK STW_SDK].softUpdate_bean.softName,[STW_BLE_SDK STW_SDK].softUpdate_bean.devieName,[STW_BLE_SDK STW_SDK].softUpdate_bean.binname];
    
    textViews.text = str_lable_text;

    [bg_view addSubview:textViews];
...
//    check_all = [STW_BLE_Protocol CRC16_1:device_check_crc16];  //加密校验
    ...
    {
        _LogLableView.text = @"File check through"; //@"文件校验通过!";
...
//        NSLog(@"文件校验通过!开始进行数据封装! - %d",check_all);
        ...
        _LogLableView.text = @"Verify the package";//@"验证安装包!";
        ...
        dispatch_after(popTime, dispatch_get_main_queue(), ^(void)
                       {
                           if (checkStartDownLoad == 0)
                           {
                               ...
                               _LogLableView.text = @"Check timed out";//@"安装包验证超时!";
                               [STW_BLE_SDK STW_SDK].Update_Now = 0;
                           }
                       });
        
        //点亮屏幕
        [STW_BLE_Protocol the_set_Activity];
...
//        NSLog(@"文件校验-->出错!");
        _LogLableView.textColor = [UIColor redColor];
        _LogLableView.text = @"Installation package validation error";//@"安装包校验出错!";
        [STW_BLE_SDK STW_SDK].Update_Now = 0;
    }
}
...
//        NSLog(@"pageNum - %d",pageNum);
        ...
        switch (pageNum)
        {
            ...
            //发送产品名称
            {
                ...
                _LogLableView.text = @"Detects the file name";//@"检测文件名称!";
            ...
            //发送软件版本
            {
                _LogLableView.text = @"Detects the software version";//@"检测软件版本!";
            ...
            //发送硬件版本
            {
                _LogLableView.text = @"Detects the device version"; //@"检测硬件!";
            ...
            //发送文件名称
            {
                _LogLableView.text = @"Detects the Update package"; //@"检测文件!";
            ...
            {
                //设置标志
                _progressUIView01.backgroundColor = VCONNEX_COLOR;//RGBCOLOR(0x00, 0xbe, 0xa4); //[UIColor greenColor];
                
                [STW_BLE_SDK STW_SDK].Update_Now = 1;
                
                //开始发送升级文件
                _LogLableView.text = @"Downloading is in progress.";
...
//                [STW_BLE_Protocol the_soft_update_page01:check_all :checkAllNum_decryption];
            ...
            {
                checkReplyAllPage = 1;
                _LogLableView.text = @"Data reception is complete.";//@"数据接收完成，等待写入!";
            }
                break;
            case 0x88:
            {
                checkReplyAllPage = 1;
                _LogLableView.text = @"The device is restarting.";//@"设备正在重启,等待重连!";
            ...
            case 0xE0:
            {
                checkDownLoadError = 1;
                _LogLableView.textColor = [UIColor redColor];
                _LogLableView.text = @"The file does not match."; //@"文件不匹配,请重新下载!";
                [STW_BLE_SDK STW_SDK].Update_Now = 0;
            }
                break;
            case 0xE1:
            {
                checkDownLoadError = 1;
                _LogLableView.textColor = [UIColor redColor];
                _LogLableView.text = @"The software version is too low."; //@"软件版本过低,请重新下载!";
                [STW_BLE_SDK STW_SDK].Update_Now = 0;
            }
                break;
            case 0xE2:
            {
                checkDownLoadError = 1;
                _LogLableView.textColor = [UIColor redColor];
                _LogLableView.text = @"Bluetooth signal is unstable."; //@"蓝牙信号不稳定,请重新下载!";
                [STW_BLE_SDK STW_SDK].Update_Now = 0;
            }
                break;
            case 0xE8:
            {
                ...
                _LogLableView.text = @"Device is writing to the program."; //@"设备正在写入程序";
                ...
                dispatch_after(popTime, dispatch_get_main_queue(), ^(void)
                               {
                                   ...
                                   {
                                       _LogLableView.textColor = [UIColor redColor];
                                       _LogLableView.text = @"Write Error.";
                                       [STW_BLE_SDK STW_SDK].Update_Now = 0;
                                   }
                               });
            }
...
//                NSLog(@"checkDownloadSuccess - %d",checkDownloadSuccess);
                ...
                if(checkDownloadSuccess > 0)
                {
                    if (data_n_now == data_n)
                    {
                        ...
                        _LogLableView.text = @"Device is restarting."; //@"设备重启等待重连";
                        
                        //需要指定为已经断开
                        [STW_BLEService sharedInstance].isBLEType = STW_BLE_IsBLETypeUpdate;
                        ...
                        dispatch_after(popTime, dispatch_get_main_queue(), ^(void)
                                       {
                                           ...
                                           if([STW_BLEService sharedInstance].isBLEStatus && checkDownloadSuccess == 1)
                                           {
                                               ...
                                               _LogLableView.text = @"Update Success.";//@"升级成功！";
                                               [STW_BLE_SDK STW_SDK].Update_Now = 0;
                                           }
                                           else if(![STW_BLEService sharedInstance].isBLEStatus && checkDownloadSuccess == 1)
                                           {
                                               checkDownloadSuccess = -1;
                                               _LogLableView.textColor = [UIColor greenColor];
                                               _LogLableView.text = @"Update Success.";
                                               [STW_BLE_SDK STW_SDK].Update_Now = 0;
                                           }
                                           else if(![STW_BLEService sharedInstance].isBLEStatus && checkDownloadSuccess == -1)
                                           {
                    ...
                    else
                    {
                        checkDownloadSuccess = -1;
                        _LogLableView.textColor = [UIColor redColor];
                        _LogLableView.text = @"Bluetooth disconnected.";//@"蓝牙断开，请重新下载！";
                        [STW_BLE_SDK STW_SDK].Update_Now = 0;
                    }
                }
                else if(checkDownloadSuccess == 0)
                {
                    checkDownloadSuccess = -1;
                    _LogLableView.textColor = [UIColor redColor];
                    _LogLableView.text = @"Bluetooth disconnected, RSSI too low."; //@"蓝牙断开，请保持设备距离!";
                    [STW_BLE_SDK STW_SDK].Update_Now = 0;
                }
            }
                break;
...
-(void)send_device_datas
{
    ...
    if (checkDownLoadError == 0)
    {
        if (data_n_now < data_n)
        {
            ...
            {
                _progressUIView02.backgroundColor = VCONNEX_COLOR;//RGBCOLOR(0x00, 0xbe, 0xa4); // [UIColor greenColor];
            }
            else if(data_n_now == check02)
            {
                _progressUIView03.backgroundColor = VCONNEX_COLOR;//RGBCOLOR(0x00, 0xbe, 0xa4); // [UIColor greenColor];
            }
            else if(data_n_now == check03)
            {
                _progressUIView04.backgroundColor = VCONNEX_COLOR;//RGBCOLOR(0x00, 0xbe, 0xa4); // [UIColor greenColor];
            }
            else if(data_n_now == check04)
            {
                _progressUIView05.backgroundColor = VCONNEX_COLOR;//RGBCOLOR(0x00, 0xbe, 0xa4); // [UIColor greenColor];
            }
...
//            }
        ...
        else
        {
            ...
            {
                _progressUIView06.backgroundColor = VCONNEX_COLOR;//RGBCOLOR(0x00, 0xbe, 0xa4); // [UIColor greenColor];
                _pageNumLableView.text = [NSString stringWithFormat:@"%d/%d",data_n_now,data_n];
                ...
                _LogLableView.text = @"Checking is in progress";//@"正在进行程序总校验";
                ...
                dispatch_after(popTime, dispatch_get_main_queue(), ^(void)
                               {
                                   ...
                                   {
                                       _LogLableView.textColor = [UIColor redColor];
                                       _LogLableView.text = @"Write Error";//@"设备写入程序失败！";
                                   }
                               });
            }
        }
